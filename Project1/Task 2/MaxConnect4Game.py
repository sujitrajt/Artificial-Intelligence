        ...
        if not self.gameBoard[0][column]:
            for i in range(5, -1, -1):
                if not self.gameBoard[i][column]:
                    ...
                    self.pieceCount += 1
                    return 1
    # place the opponent move in the requested column 
    def opponentPiece(self,column,opponent):
        if not self.gameBoard[0][column]:
            for i in range(5, -1 ,-1):
                if not self.gameBoard[i][column]:
                    ...
                    self.pieceCount += 1
                    return 1
    #implementation of plain min max algorithm 
    # def miniMax(self,depth):
    #     getCurrentGameBoard = deepcopy(self.gameBoard)
    #     values = []
    #     for i in range (0,7):
    #         if self.playPiece(i):
    #             if self.checkPieceCount() == 42 or self.depth == 0:
    #                 self.gameBoard = deepcopy(getCurrentGameBoard)
    #                 return i
    #             else :
    #                 value = self.getMin(self.gameBoard)
    #                 utility_value[i] = value
    #                 self.gameBoard = deepcopy(getCurrentGameBoard)
    #     for _ in utility_value:
    #         values.append(_)
